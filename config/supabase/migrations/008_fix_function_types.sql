-- Fix data types to match the actual table schema

DROP FUNCTION IF EXISTS public.get_user_readings();
DROP FUNCTION IF EXISTS public.create_reading_entry(text, integer, date, integer, uuid, uuid);
DROP FUNCTION IF EXISTS public.delete_reading_entry(uuid);
DROP FUNCTION IF EXISTS public.get_or_create_book(text, text, text, integer, text, text, text, text, uuid);
...
-- Get user's readings (with correct types)
...
-- Create reading entry (with correct types)
